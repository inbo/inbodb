...
This function queries the INBOVEG database for relation information on
recordings for one or more surveys based on Parent
(classic-chain/bucket) and Child (classic) relationship.
...
A dataframe with variables `RecordingId`, `Child_GIVID` (unique
`RecordingGIVID`), `Child_UserRef` (`UserReference`), `ParentId`
(`RecordingId`), `Parent_GIVID` (unique `RecordingGIVID`) and
