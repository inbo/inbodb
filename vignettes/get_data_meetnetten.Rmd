...
The Flemish species monitoring programme [meetnetten.be](www.meetnetten.be) consists of set of monitoring schemes for priority species in Flanders, the northern part of Belgium. 
Each monitoring scheme consists of a number of monitoring sites at which target species are counted using a standardized protocol. 
The count data is stored in the meetnetten database which is maintained by the Nature and Forest Research Institute (INBO).
We present some functions to easily access the data from the meetnetten database.
...
+ locations and sublocations (for example different sections of a transect) of the monitoring schemes
+ visits (count events)
+ observations
  
# Packages and connection

The main functions that we will use in this tutorial all start with `get_meetnetten_*`. 
These functions are made available by loading the `inbodb` package.
We also load the `tidyverse` and `sf` package
...
As an INBO employee, you should make sure you have reading-rights for the INBO DWH Server, otherwise place an ICT-call.
...
```{r, fig.cap = "Arenbergpolder location of the Argusvlinder monitoring scheme and the different sublocations", fig.width = 7}
...
```{r, fig.cap = "All locations from the dragonfly monitoring schemes", fig.width = 7}
...
The function `get_meetnetten_visits` provide all count events for one or more 
monitoring schemes.
...
The function `get_meetnetten_observations` provide all count events for one or more 
monitoring scheme.
