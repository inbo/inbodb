...
```{r, eval=TRUE, echo=FALSE}
dbGetQuery(con, "select Lijst, PublicatieJaar, NaamWet, NaamNed,
...
get_taxonlijsten_items(con, version = "all",
                       list = "rode lijst van de dagvlinders",
                       taxon = "%adippe%", original = TRUE) %>%
  select(c("Lijst", "Publicatiejaar", "Naamwet_origineel", "NaamNed_origineel",
...
The taxonlistitems that appear on a taxonlist version may or may not have associated features. Typical examples of features are the Red List category or the Annex on which a taxonlistitem is listed. These are the features that are currently defined in the database:

```{r, eval=TRUE, echo=FALSE}
dbGetQuery(con, "select distinct Kenmerkcode, Kenmerk from
           [dbo].[vw_Kenmerk]") %>%
  kable() %>%
  kable_styling(font_size = 9)
```
...
```{r, eval=TRUE, echo=FALSE}
dbGetQuery(con, "select Kenmerkcode, kenmerk, kenmerkwaardecode,
...
```{r, eval=TRUE, echo=FALSE}
dbGetQuery(con, "select Lijst, PublicatieJaar, NaamWet_interpretatie,
