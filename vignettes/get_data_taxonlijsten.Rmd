...
vignette: >
  %\VignetteIndexEntry{get_data_taxonlijsten}
...
If you notice data issues or there is a taxonlist missing that you would like to be included in the database, please contact dirk.maes@inbo.be
...
select('Taxonlijst', 'PublicatieJaar', 'Criteria', 'Validering', 'Vaststelling')
```
...
```{r, eval=TRUE, echo=TRUE}
RL_butterfly <- get_taxonlijsten_features(con, version = 'all', list = '%rode%dagvlinders%', collect = TRUE)
```
...
```{r, eval=TRUE, echo=TRUE}
no_feature <- get_taxonlijsten_features(con, list = '%SBP%')
```
...
```{r, eval=TRUE, echo=TRUE}
jachtdecreet <- get_taxonlijsten_items(con, list =  'Jachtdecreet', collect = TRUE)
```

Get all taxa on category 2 of 'Soortenbesluit':

```{r, eval=TRUE, echo=TRUE}
soortenbesluit_cat2 <- get_taxonlijsten_items(con, list =  'soortenbesluit', feature = 'cat2')
```

Get all taxonlist that include 'Gentiaanblauwtje' (you can search for scientific as well as Dutch names):

```{r, eval=TRUE, echo=TRUE}
gentiaanblauwtje <- get_taxonlijsten_items(con, taxon = 'Gentiaanblauwtje', collect = TRUE)
```
...
```{r, eval=TRUE, echo=TRUE}
critical <- get_taxonlijsten_items(con, feature = 'CR', list = 'Rode lijst%vaatpl%')
```

Get original and interpreted Cicindela taxa from list 'Soortenbesluit'

```{r, eval=TRUE, echo=TRUE}
cicindela <- get_taxonlijsten_items(con, list = 'Soortenbesluit', taxon = '%Cicindela%', original = TRUE) %>%
select(c('Naamwet_origineel', 'NaamNed_origineel', 'Naamwet_interpretatie', 'NaamNed_interpretatie'))
```

Compare red list status on multiple list versions (requires tidyverse):

```{r, eval=TRUE, echo=TRUE}
redlist_evolution <- get_taxonlijsten_items(con, version = 'all', list = 'rode lijst van de dagvlinders') %>%
select(c('Lijst','Publicatiejaar', 'Naamwet_interpretatie', 'NaamNed_interpretatie', 'KenmerkwaardeCode')) %>%
pivot_wider(names_from = 'Publicatiejaar', values_from = 'KenmerkwaardeCode')
```


## More complex queries
These functions give most of the relevant basic information that is available in D0156_00_Taxonlijsten.
If more detailed information is needed `dplyr` is the magic word. 

# Database philosophy

The diagram below shows the core tables of the database that are essential to grasp the philosophy behind the datamodel. Full datamodel with short descriptions of tables and fields (in Dutch) is provided at the end of this document.

![Taxonlijsten datamodel 00.04.00](images/taxonlijsten_datamodel_core.png)

## Taxonlijst

A taxonlist is a list of taxa that belong together for one reason or another. That reason can be legal (Habitats Directive, Birds Directive, Species Decree, Hunting Decree, etc.), thematic (ecoprofiles, etc.) or somewhere in between (Red lists, Species Protection Plans). This may seem pretty straightforward at first glance, but the concept of a taxonlist is susceptible to interpretation.

## TaxonlijstVersie

Taxonlists may be revised or updated over time. A typical example is the 10-yearly revision of the Red Lists. In that case we are talking about different versions of a taxonlist. 

A taxonlist has at least one taxonlist version, but it can have multiple versions. As a rule of thumb a new taxonlist version can be related to a new publication. An example is the Union list of invasive alien species. This list is regularly amended. Each amendment is considered a new taxonlist version because it can be traced back to a citable, consolidated version:

```{r, eval=TRUE, echo=FALSE}
dbGetQuery(con, "select tl.id as taxonlijstid
...
All elements (taxa) on a taxonlist version are called taxonlistitems. These are the taxon(group) names **as they were originally published**. Often this will be species names, but it might as well be a description of a taxongroup that requires further interpretation. The Species Decree (Soortenbesluit) for example, includes many descriptive items such as:
...
It also happens that names are published incorrectly (typos, errors in the authors, incorrect scientific or Dutch name, etc.). Even then, the names are copied literally as they were published, including errors. An example: In the Blueprints for species monitoring in Flanders (a list of Natura 2000 and other Flemish priority species) we find Veenmosorchis - *Dactylorhiza sphagnicola*. Two different species are confused here, namely Veenorchis - *Dactylorhiza sphagnicola* and Veenmosorchis - *Hammarbya paludosa*. We include the incorrect version in the Taxonlistitem table, the correct interpretation only takes place afterwards.

A Dutch name may be published without a scientific name or vice versa, scientific names are published either with or without an author's name, it all doesn't matter: we just copy things as they appear in the original list. Hence, the taxonlistitem table always allows you to recall the list exactly as it was published. There is no direct link with the Taxon table, that link will be made in the table TaxonlijstItemInterpretatie.

## TaxonlijstitemInterpretatie

This table links the name as it was originally published (Taxonlistitem) with one or more taxa. We call this link the interpretation of the original name. Usually the interpretation will be unambiguous and one-to-one, but in other cases a translation must be done. For example, the reindeer mosses from the example above refer to 6 species occurring in Flanders:

```{r, eval=TRUE, echo=FALSE}
dbGetQuery(con, "select Lijst, PublicatieJaar, NaamWet, NaamNed, NaamWet_interpretatie, Auteur, NaamNed_interpretatie, taxongroep
from [dbo].[vw_Taxonlijstitem_detail]
where naamwet like '%cladina%' and Lijst = 'Soortenbesluit'")
```

The erroneuos publication of Veenmosorchis - *Dactylorhiza sphagnicola* is interpreted as follows:

```{r, eval=TRUE, echo=FALSE}
dbGetQuery(con, "select Lijst, PublicatieJaar, NaamWet, NaamNed, NaamWet_interpretatie, Auteur, NaamNed_interpretatie, taxongroep
from [dbo].[vw_Taxonlijstitem_detail]
where naamwet like 'Dactylorhiza sphagnicola' and Lijst = 'Soortenmeetnetten'")
```
...
This is the reference table with the taxa of this database. When managing this table, a number of important principles must be taken into account. The taxon model has been deliberately kept very simple. There are no taxonomic hierarchies, nor is it possible to define synonyms between taxa. It is strongly discouraged to maintain more than one record in the taxon table for a particular taxon. Let's take the High Brown Fritillary (Bosrandparelmoervlinder) as an example. This is the corresponding taxon record:
...
But that is not how it works. For each taxon there should be only one record, i.e. the name that is considered by the administrator of the taxon table as the currently accepted scientific and currently accepted Dutch name. Synonyms can of course occur as an original taxon list item. For example, we see that in the first version of the Red List of Butterflies the old Dutch name 'Adippevlinder' was used. The second version only contained Dutch names. They are all linked to the single entry Fabriciana adippe - Bosrandparelmoervlinder in the taxon table:

```{r, eval=TRUE, echo=FALSE}
get_taxonlijsten_items(con, version = 'all', list = 'rode lijst van de dagvlinders', taxon = '%adippe%', original = TRUE) %>%
  select(c('Lijst', 'Publicatiejaar', 'Naamwet_origineel', 'NaamNed_origineel', 'Naamwet_interpretatie', 'NaamNed_interpretatie'))
```
...
The taxonlistitems that appear on a taxonlist version may or may not have associated features. Typical examples of features are the Red List category or the Annex on which a taxonlistitem is listed. Currently these features are defined in the database:

```{r, eval=TRUE, echo=FALSE}
dbGetQuery(con, "select distinct Kenmerkcode, Kenmerk from [dbo].[vw_Kenmerk]")
```
...
```{r, eval=TRUE, echo=FALSE}
dbGetQuery(con, "select Kenmerkcode, kenmerk, kenmerkwaardecode, kenmerkwaarde from [dbo].[vw_Kenmerk]")
```

## TaxonlijstItemAttribuut

This is the relation table linking a taxonlistitem to its feature values. A taxonlistitem can have multiple feature values for the same feature. For example: the moor frog is linked to 3 habitat types on the taxon list of habitattypical fauna species:

```{r, eval=TRUE, echo=FALSE}
dbGetQuery(con, "select Lijst, PublicatieJaar, NaamWet_interpretatie, NaamNed_interpretatie, Kenmerk, KenmerkwaardeCode, Kenmerkwaarde
from [dbo].[vw_Taxonlijstitem_detail] where NaamNed_interpretatie = 'Heikikker' and Lijst = 'Habitattypische faunasoorten' and LaatsteVersie = 1")
```


# Full datamodel

![Taxonlijsten datamodel 00.04.00](images/taxonlijsten_datamodel.png)
